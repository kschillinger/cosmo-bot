/*
 * font_5x7.c — bitmap data for the 5x7 ASCII font.
 *
 * Each row below is 5 bytes encoding one glyph, columns left→right.
 * Within each byte: bit 0 = topmost pixel, bit 6 = bottom of the 7-row cell.
 *
 * The shapes are conventional 5x7 forms (the same skeletons used in countless
 * embedded fonts since the 70s); at this resolution there is essentially one
 * legible way to draw each Latin glyph.
 */

#include "font_5x7.h"

static const uint8_t k_font[96][5] = {
